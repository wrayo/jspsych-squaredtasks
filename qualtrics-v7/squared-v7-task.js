...
var squaredTaskConfig = window.squaredTaskConfig || {};
var squaredTaskHooks = window.squaredTaskHooks || {};

function resolveDisplayElement(displayElement) {
	if (!displayElement) {
		return undefined;
	}
...
var jsPsych = initJsPsych({
	display_element: resolveDisplayElement(squaredTaskConfig.displayElement),
	on_finish: function() {
		var payload = buildExperimentPayload();
		window.squaredTaskLastRun = payload;

		if (typeof squaredTaskHooks.onFinish === "function") {
			squaredTaskHooks.onFinish(jsPsych, payload);
		}
	}
});
...
var practice_duration = typeof squaredTaskConfig.practiceDurationMs === "number" ? squaredTaskConfig.practiceDurationMs : 30000; // duration of practice
var main_duration = typeof squaredTaskConfig.mainDurationMs === "number" ? squaredTaskConfig.mainDurationMs : 90000; // duration of main task

//general variables for use throughout the experiment
var online = typeof squaredTaskConfig.online === "number" ? squaredTaskConfig.online : 1; // Numeric: 0 indicating that task is being run locally, 1 indicating that the task is being run through a platform
var subject = squaredTaskConfig.participantId || null; // Subject ID
...
var use_fullscreen = squaredTaskConfig.fullscreenMode === true;
var finish_button_label = squaredTaskConfig.finishButtonLabel || "Continue";
...
function getCompletedTrials(taskName, practiceFlag) {
...
function getSafeScoreAfterTrial(taskName, practiceFlag) {
	var completedTrials = getCompletedTrials(taskName, practiceFlag);
	return completedTrials.count() ? completedTrials.last(1).values()[0].score_after_trial : null;
}
...
var get_participant_id = {
	...
	on_finish: function(data) {
		setParticipantId(data.response.participant_id);
	  }
}
...
var conclusion_stroop = {
	...
	on_start: function() {
		...
		jsPsych.data.get().filter({task: "stroop", practice: 1}).addToAll({
			score_final: getSafeScoreAfterTrial("stroop", 1),
		...
		jsPsych.data.get().filter({task: "stroop", practice: 0}).addToAll({
			score_final: getSafeScoreAfterTrial("stroop", 0),
...
var conclusion_flanker = {
	...
	on_start: function() {
		...
		jsPsych.data.get().filter({task: "flanker", practice: 1}).addToAll({
			score_final: getSafeScoreAfterTrial("flanker", 1),
		...
		jsPsych.data.get().filter({task: "flanker", practice: 0}).addToAll({
			score_final: getSafeScoreAfterTrial("flanker", 0),
...
var conclusion_simon = {
	...
	on_start: function() {
		...
		jsPsych.data.get().filter({task: "simon", practice: 1}).addToAll({
			score_final: getSafeScoreAfterTrial("simon", 1),
		...
		jsPsych.data.get().filter({task: "simon", practice: 0}).addToAll({
			score_final: getSafeScoreAfterTrial("simon", 0),
...
function safeMean(dataCollection, columnName) {
	if (dataCollection.count() === 0) {
		return null;
	}

	return dataCollection.select(columnName).mean();
}

function summarizeCondition(taskName, practiceFlag, conditionNumber) {
	var correctTrials = jsPsych.data.get().filter({
		task: taskName,
		practice: practiceFlag,
	...
	var incorrectTrials = jsPsych.data.get().filter({
		task: taskName,
		practice: practiceFlag,
		condition: conditionNumber,
		accuracy: 0,
	...
	var allTrials = jsPsych.data.get().filter({
		task: taskName,
		practice: practiceFlag,
		condition: conditionNumber,
		timeout: 0
	...
	return {
		score: correctTrials.count() - incorrectTrials.count(),
		mean_rt: safeMean(allTrials, "rt"),
		trial_count: allTrials.count()
	};
}

function summarizeTaskBlock(taskName, practiceFlag) {
	var completedTrials = jsPsych.data.get().filter({
		task: taskName,
		practice: practiceFlag,
		timeout: 0
	});
	var lastCompletedTrial = completedTrials.count() ? completedTrials.last(1).values()[0] : null;

	return {
		completed_trial_count: completedTrials.count(),
		score_final: lastCompletedTrial ? lastCompletedTrial.score_after_trial : null,
		meanrt_final: safeMean(completedTrials, "rt"),
		condition_1: summarizeCondition(taskName, practiceFlag, 1),
		condition_2: summarizeCondition(taskName, practiceFlag, 2),
		condition_3: summarizeCondition(taskName, practiceFlag, 3),
		condition_4: summarizeCondition(taskName, practiceFlag, 4)
	};
}
...
function buildExperimentPayload() {
	var payload = {
		participant_id: subject,
		online: online,
		trial_count: jsPsych.data.get().count(),
		total_scores: {
			stroop: total_stroop,
			flanker: total_flanker,
			simon: total_simon
		},
		summary: {
			stroop: {
				practice: summarizeTaskBlock("stroop", 1),
				main: summarizeTaskBlock("stroop", 0)
			},
			flanker: {
				practice: summarizeTaskBlock("flanker", 1),
				main: summarizeTaskBlock("flanker", 0)
			},
			simon: {
	...
	if (squaredTaskConfig.includeTrialData === true) {
		payload.all_data = jsPsych.data.get().values();
	}
...
var conclusion = {
	type: jsPsychHtmlButtonResponse,
	stimulus: function() { return '<p style="font-size:25px;">You earned the following points in the three tasks:</p>' +
...
if (prompt_for_participant_id) {
	timeline.push(get_participant_id);
}

if (prompt_for_location) {
	timeline.push(get_location);
}
...
timeline.push(stroop_task, flanker_task, simon_task, conclusion);
